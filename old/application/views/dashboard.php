				...
				<div class="breadcrumb-menu">
					<nav aria-label="Breadcrumbs" class="breadcrumb-trail breadcrumbs">
						<ul class="trail-items">
							...
							<li class="trail-item trail-end"><span>My History</span></li>
						</ul>
					</nav>
				</div>
			</div>
	...
	<div class="faq-area pt-120 pb-120">
		<div class="container">
			<div class="faq-wrapper wow fadeInUp" data-wow-delay=".3s">
				<div class="gm-faq gm-faq-1column">
						...
						<div class="gm-faq-group">
							...
							if (!empty($checkout)) {
								foreach ($checkout as $row) {
									$checkout_product = getRowById('checkout_product', 'checkoutid', $row['id']);
									$checkout_count = getNumRows('checkout_product', ['checkoutid' => $row['id']]);
									$productStatus = ['Placed', 'Confirmed', 'Processed', 'Delivered', 'Cancel']
							?>
									<div class="accordion-item">
										<h2 class="accordion-header" id="heading<?= $row['id'] ?>">
											<button class="accordion-button <?= ($i != 0) ? 'collapsed' : '' ?> row" type="button" data-bs-toggle="collapse" data-bs-target="#collapse<?= $row['id'] ?>" aria-expanded="true" aria-controls="collapse<?= $row['id'] ?>">

												<div class="col-3">Order #PP-<?= $row['id'] ?></div>
												<div class="col-3"><?= $checkout_count ?> &nbsp; Products </div>

											</button>
										</h2>
										<div id="collapse<?= $row['id'] ?>" class="accordion-collapse collapse  <?= ($i != 0) ? '' : 'show' ?>" aria-labelledby="heading<?= $row['id'] ?>" data-bs-parent="#accordionExample">
											<div class="accordion-body">
												<div class="table-content table-responsive">
													<table class="table">
														...
														<tbody>
															<?php
															if ($checkout_product) {
																foreach ($checkout_product as $items) {
															?>
																	<tr>
																		<td class="product-thumbnail">
																			<a href="#"><img src="<?= setImage($items['product_img'], 'uploads/product/'); ?>" alt="Papaplants" style="height:60px;"></a>
																		</td>
																		<td class="product-name">
																			<a href="#"><?= $items['product_name']; ?></a>
																		</td>
																		<td class="product-price">
																			<span class="amount"> &#8377; <?= $items['product_price'] ?></span>
																		</td>
																		<td class="product-price">
																			<span class="amount"> <?= $items['product_quantity'] ?></span>
																		</td>

																		<td class="product-subtotal">
																			<span class="amount">&#8377; <?= $items['product_quantity'] * $items['product_price'] ?></span>
																		</td>
												...
												<div class="row shop-tracking-status">
													<div class="col-md-6">
														<h5>Date: <?= convertDatedmy($row['create_date']) ?></h5>

														<h5>Payment status: <?= ($row['payment_status'] == 0) ? '<span class="badge bg-warning">Pending</span>' : '' ?> <?= ($row['payment_status'] == 1) ? '<span class="badge bg-success">Paid</span>' : '' ?> <?= ($row['payment_status'] == 2) ? '<span class="badge bg-danger">Payment Failed</span>' : '' ?> <br>
															<?= ($row['payment_status'] == 1) ? 'Transaction Id: ' . $row['razorpay_payment_id'] : '' ?></h5>
													</div>
													<div class="col-md-6">
														<div class="well">
															<div class="order-status">
																...
																<div class="order-status-timeline">
																	...
																	<?= (($row['status'] == 0) ? 'c0' : (($row['status'] == 1) ? 'c1' : (($row['status'] == 2) ? 'c2' : (($row['status'] == 3) ? 'c3' : (($row['status'] == 4) ? 'c4' : ''))))) ?>"></div>
																</div>
