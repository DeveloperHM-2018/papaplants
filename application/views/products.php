							...
							<div class="sidebar-category-list">
								...
								if (!empty($category)) {
									foreach ($category as $cat_name) {
										$count = getNumRows('pp_product_category', ['category_id' => $cat_name['id']])
								?>
										<div class="form-check product" style="margin-bottom: 0rem !important;">
											<input class="form-check-input common_selector category" type="checkbox" value="<?= $cat_name['id']; ?>" id="flexCheckDefault<?= $cat_name['id']; ?>" <?= ($category_id == $cat_name['meta_url']) ? 'Checked' : '' ?>>
											<label class="form-check-label text-capitalize" for="flexCheckDefault<?= $cat_name['id']; ?>">
												<?= $cat_name['name']; ?> <span class="category-item-number"> (<?= $count ?>)</span>
											</label>
								...
								if (!empty($sub_category)) {
									foreach ($sub_category as $sub_info) {
										$count = getNumRows('pp_product_category', ['category_id' => $sub_info['id']])
								?>
										<div class="form-check" style="margin-bottom: 0rem !important;">
											<input class="form-check-input common_selector sub_category" type="checkbox" value="<?= $sub_info['id']; ?>" id="flexCheckDefault<?= $sub_info['id']; ?>" <?= ($sub_category_id == $sub_info['meta_url']) ? 'Checked' : '' ?> <?= ($category_id == $sub_info['meta_url']) ? 'Checked' : '' ?>>
											<label class="form-check-label text-capitalize" for="flexCheckDefault<?= $sub_info['id']; ?>">
												<?= $sub_info['name']; ?> <span class="category-item-number"> (<?= $count ?>)</span>
											</label>
										</div>
								<?php
									}
