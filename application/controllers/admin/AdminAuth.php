...
class AdminAuth extends CI_Controller
{
	...
	function test()
	{
		...
		$get = $this->CommonModel->getAllRows('pp_product_images');

		if ($get) {
			foreach ($get as $list) {

				$img = explode(",", $list['image_file']);


				$fileNames = array_map('basename', $img);
				...
				foreach ($fileNames as $fileName) {
					...
					if (count(explode(".", $fileName)) == 2) {
						...
						$this->CommonModel->insertRow('product_image', ['product_id' => $list['product_id'], 'image_path' => $fileName]);
